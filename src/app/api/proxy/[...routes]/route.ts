  ...
  try {
        ...
        try {
          ...
          if (path.includes('/attend/enroll')) {
            ...
            if (jsonBody.userId && jsonBody.courseId) {
              ...
              if (!requestBody.course_id || requestBody.course_id.trim() === '') {
    ...
    try {
      ...
      const isNetworkError = 
        ...
        (typeof fetchError.cause?.message === 'string' && fetchError.cause.message.includes('closed'));
                             
      if (isNetworkError) {
        return NextResponse.json(
          { 
            error: "Network connectivity issue. Unable to reach the API server.", 
            details: "The connection to the backend API server was closed unexpectedly."
          },
